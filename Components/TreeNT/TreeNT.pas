...
// Version     - 2.3.7
// Last Change - 26. April 1999
//
// written by Dipl. Ing. Mike Lischke (Lischke@hotmail.com)
//
// SQL features added by Paul Petsevitch (paul@bigit.com, pasha@junik.lv)
//
...
uses Windows, Classes,
     {$ifdef DFS_COMPILER_4} ImgList, {$endif}
//     {$ifndef DFS_DELPHI_4_UP} CommCtrl98, {$else}
     CommCtrl, //{$endif}
     Controls, DsgnIntf, ExtCtrls,
     Forms, Graphics, Messages, SysUtils, Menus,
     ComCtrls, db, dbtables;


{$ifdef DFS_COMPILER_4}
  const CDRF_NOTIFYITEMERASE = $00000080;
{$endif}

const // indices for check state images used for checking
...
const DefaultOptions = [toAutoExpand, toAutoScroll, toEvenHeight, toHideSelection, toShowButtons,
...
type
  ...
  TNodeState      = (nsChecked, nsCut, nsDisabled, nsDropHilited, nsExpanded,
                     nsExpandedOnce, nsExpandPartial, nsFocused, nsGrayed, nsHot,
                     nsIndeterminate, nsMarked, nsSelected);
  ...
  PNodeInfo = ^TNodeInfo;
  TNodeDataType = (ndInt, ndFloat, ndBmpIndex);
  ...
  TChangeOp = (opChange, opAdd, opDelete, opInsert);
  ...
  TChangeRec   = record
    ...
    bDATA: smallint;
    sDATA: shortstring;
    iCatName: shortstring;
    bCatName: shortstring;
    StructureLocked: boolean;
    iCatID: integer;
    bCatID: smallint;
  end;

  TChangesList = class(TObject)
  public
    List     : array of TChangeRec;
    constructor Create;
    destructor  Destroy; override;
    procedure   Clear;
    procedure   Add(AN: TTreeNTNode; AOp:TChangeOp);
    function    GenSQL(AIndex: integer; const DTable: string): string;
  end;
  ...
  private
    ...
    FbDATA: smallint;                   // *
    FsDATA: shortstring;                // *
    FiCatName: shortstring;             // *
    FbCatName: shortstring;             // *
    ...
    procedure SetiDATA(AiDATA: integer);          // *
    procedure SetfDATA(AfDATA: double);           // *
    procedure SetbDATA(AbDATA: smallint);             // *
    procedure SetsDATA(AsDATA: shortstring);           // *
    procedure SetiCatName(AiCatName: shortstring);     // *
    procedure SetbCatName(AbCatName: shortstring);     // *
    procedure SetStructureLocked(ToState: boolean);      // *
    procedure SetiCatID(AiDATA: integer);          // *
    procedure SetbCatID(AbDATA: smallint);             // *
  ...
  public
    procedure SetStructureLock(ToState: boolean);      // *
    procedure SetSingleCheckType(ToType: TCheckType);  // *
    ...
    procedure CopyNode;                                    // *
    procedure PasteNodesTo(Destination: TTreeNTNode);      // *

    procedure MakeVisible;
    procedure MoveTo(Destination: TTreeNTNode; Mode: TNodeAttachMode);

    property iDATA: integer read FiDATA write SetiDATA;                  // *
    property fDATA: double read FfDATA write SetfDATA;                   // *
    property bDATA: smallint read FbDATA write SetbDATA;                     // *
    property sDATA: shortstring read FsDATA write SetsDATA;              // *
    property iCatName: shortstring read FiCatName write SetiCatName;     // *
    property bCatName: shortstring read FbCatName write SetbCatName;     // *
    property StructureLocked: boolean read FStructureLocked
                                      write SetStructureLocked;          // *
    property iCatID: integer read FiCatID write SetiCatID;               // *
    property bCatID: smallint read FbCatID write SetbCatID;              // *
    ...
    property CheckType: TCheckType read FCheckType write SetSingleCheckType;
    property Color: TColor read GetColor write SetColor;
    property Count: Integer read GetCount;
    property Cut: Boolean read GetCut write SetCut;
    property Data: Pointer read FaData write SetData;
  ...
  private
    ...
    function  GetItemID: string;                 // *
    procedure SetQueryValue(a:TDataSet);         // *
    procedure SetMenuEnabled(abState: boolean);  // *
    procedure OnStdMenuPopup(Sender: TObject);   // *
    ...
    procedure SetChanged(AState:boolean);
  protected
    FChangeTimer: TTimer;
    ...
    function  GetDragImages: {$ifdef DFS_COMPILER_4} TDragImageList {$else} TCustomImageList {$endif}; override;
  ...
  public
    DragNode, TargetNode: TTreeNTNode;
    FCopiedNode: TTreeNTNode;       // *
    SQL,rSQL: TStrings;
    ExecSQL: TCustProc;
    ...
    procedure LoadFromDB(const TableName: String; const WhereClause: string='');
    procedure SaveToDB(const TableName: String; const WhereClause: string=''; const LimitNode: string='');
    procedure SaveToFile(const TableName: String; const WhereClause: string=''; const LimitNode: string=''); overload;
    procedure SaveToFile(const FileName: String; Complete: Boolean); overload;
    ...
    property _ItemID: string read GetItemID write FCurItemID;    // *
    property _ItemConnQuery: TDataSet read FConnQuery write SetQueryValue;          // *
    property _MenuEnabled: boolean read FMenuEnabled write SetMenuEnabled;          // *
    property _AdvMenuEnabled: boolean read FAdvMenuEnabled write FAdvMenuEnabled;   // *
    property Changed: boolean read FTreeChanged write SetChanged;                   // *
  ...
  published
    property _ItemID;                          // *
    property _ItemConnQuery;                   // *
    property _MenuEnabled;                     // *
    property _AdvMenuEnabled;                  // *
...
uses Consts, ComStrs, Dialogs, Printers;

var CheckImages : TImageList;
    CurrentLine : Integer;
...
procedure TChangesList.Add(AN: TTreeNTNode; AOp:TChangeOp);
begin
end;
...
destructor TChangesList.Destroy;
begin
  SetLength(List, 0);
  inherited;
end;

procedure TChangesList.Clear;
begin
  SetLength(List, 0);
end;

function TChangesList.GenSQL(AIndex: integer; const DTable: string): string;
begin
//  TChangeOp = (opChange, opAdd, opDelete, opInsert);
{  if not (AIndex in [0..high(List)])
    then exit;
  with List[AIndex] do
  case Op of
    opAdd : Result:=Format('insert into %s (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)'#13#10+
                'values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s")',
        [DTable, NodeID, Text, byte(CheckType), byte(CheckState), iDATA, fDATA,
         bDATA, sDATA, byte(StructureLocked) or byte(Index) shl 1,
         iCatID, iCatName, bCatID, bCatName]));
    opChange : Result:=Format('update %s set Caption="%s", CheckType=%d, '#13#10+
                'CheckState=%d, iDATA=%d, fDATA=%5.4f, bDATA=%d, sDATA="%s", AddDATA=%d, '#13#10+
                'iCatID=%d, iCatName="%s", bCatID=%d, bCatName="%s"'#13#10+
                'where NodeID="%s"',
        [DTable, Text, byte(CheckType), byte(CheckState), iDATA, fDATA,
         bDATA, sDATA, byte(StructureLocked) or byte(Index) shl 1,
         iCatID, iCatName, bCatID, bCatName, NodeID]));
    opDelete : Result:=Format('delete from %s where NodeID="%s" and NodeID like "%s"'#13#10,
                              [DTable, NodeID, NodeID+'%'])+
                       Format('update %s where NodeID="%s" and NodeID like "%s"'#13#10, [DTable, NodeID, NodeID+'%'])
        Format('insert into %s (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)'#13#10+
                'values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s")',
        [DTable, NodeID, Text, byte(CheckType), byte(CheckState), iDATA, fDATA,
         bDATA, sDATA, byte(StructureLocked) or byte(Index) shl 1,
         iCatID, iCatName, bCatID, bCatName]));
    opInsert : Result:=Format('insert into %s (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)'#13#10+
                'values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s")',
        [DTable, NodeID, Text, byte(CheckType), byte(CheckState), iDATA, fDATA,
         bDATA, sDATA, byte(StructureLocked) or byte(Index) shl 1,
         iCatID, iCatName, bCatID, bCatName]));
...
procedure TTreeNTNode.SetiCatID(AiDATA: integer);          // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FiCatID:=AiDATA;
  ChangesList.Add(Self, opChange);
end;

procedure TTreeNTNode.SetbCatID(AbDATA: smallint);          // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FbCatID:=AbDATA;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetiDATA(AiDATA: integer);          // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FiDATA:=AiDATA;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetfDATA(AfDATA: double);           // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FfDATA:=AfDATA;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetbDATA(AbDATA: smallint);             // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FbDATA:=AbDATA;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetsDATA(AsDATA: shortstring);           // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FsDATA:=AsDATA;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetiCatName(AiCatName: shortstring);     // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FiCatName:=AiCatName;
  ChangesList.Add(Self, opChange);
...
procedure TTreeNTNode.SetbCatName(AbCatName: shortstring);     // *
begin
  if assigned(FOwner) then TreeView.Changed:=true;
  FbCatName:=AbCatName;
  ChangesList.Add(Self, opChange);
...
function TTreeNTNode.GetItemID: string;
  function IDchar(idvalue: byte): char;
  begin
    case idvalue of
      0..9 : Result:=chr(ord('0')+idvalue);
      10..35 : Result:=chr(ord('A')+idvalue-10);
      36..61 : Result:=chr(ord('a')+idvalue-36);
      else Result:=chr(ord('{')+idvalue-62);
    end;
  end;
  function GetNodeItemID(N:TTreeNTNode): string;
  begin
    if N=nil
      then Result:=''
      else if N.Level=0
             then Result:=IDchar(N.Index)
             else if N.Parent=nil
                    then Result:=''
                    else if (N.Parent.Level=0) and (TNTRootNODE<>'')
                           then Result:=IDchar(N.Index)
                           else Result:=GetNodeItemID(N.Parent)+IDchar(N.Index);
  end;
begin
  if (TNTRootNODE<>'') and (Self.Level=0)
    then Result:=TNTRootNODE
    else Result:=TNTRootNODE+GetNodeItemID(Self);
end;
...
begin
  if FEnabled then
    case {FParent.}FCheckType of
      ctCheckBox:
        if FCheckState = csUnchecked
          then SetCheckState(csChecked)
  ...
  begin
    case {FParent.}FCheckType of
      ...
      // propagate state up to the parent
          if (FParent <> FOwner.FRoot) and
             (FParent.FCheckType = ctCheckBoxGrayed)
            then CheckParentCheckState(FParent);
        end;
      ...
      ctCheckBoxGrayed:
        begin
          ...
          if FCheckType in [ctCheckBoxGrayed] then
          case Value of
            csUnchecked:
      ...
      ctRadioButton:
        if Value = csChecked then
        begin
          FCheckState := Value;
          ChangesList.Add(Self, opChange);
          for I := 0 to FParent.Count - 1 do
            if (FParent.Item[I].CheckType=ctRadioButton)
            then if FParent.Item[I] = Self
                   then TreeView.SetCheckImage(Self, ckRadioChecked)
                   else begin
                     if FParent.Item[I].FCheckState = csChecked
                       then ChangesList.Add(FParent.Item[I], opChange);
                     FParent.Item[I].FCheckState := csUnchecked;
                     if FParent.Item[I].Enabled
                          then TreeView.SetCheckImage(FParent.Item[I], ckRadioEmpty)
                          else TreeView.SetCheckImage(FParent.Item[I], ckRadioDisabled);
                   end;
        end
        else for I := 0 to FParent.Count - 1 do
            if (FParent.Item[I].CheckType=ctRadioButton)
            then if FParent.Item[I].CheckState <> csChecked
                   then if FParent.Item[I].Enabled
                          then TreeView.SetCheckImage(FParent.Item[I], ckRadioEmpty)
                          else TreeView.SetCheckImage(FParent.Item[I], ckRadioDisabled);
    end;
    TreeView.Check(Self);
      ...
      ctRadioButton:
        begin
          ...
          begin
            ...
            if Item[I].Enabled and (Item[I].FCheckState = csChecked) then
            begin
...
begin
  ...
  if not Cancel
    then if assigned(FOwner) then TreeView.Changed:=true;
end;
...
//------------------------------------------------------------------------------
procedure TTreeNTNode.CopyNode;
begin
  TreeView.FCopiedNode:=Self;
end;

//------------------------------------------------------------------------------
procedure TTreeNTNode.PasteNodesTo(Destination: TTreeNTNode);
var LastFocused   : TTreeNTNode;
    OldOnChanging : TTVChangingEvent;
    OldOnChange   : TTVChangedEvent;
    S, D: TTreeNTNode;
    i, StartLevel, CurLevel: smallint;

begin
  if not assigned(Destination) and
    (Destination.TreeView <> TreeView) then Exit;
  OldOnChanging := TreeView.OnChanging;
  OldOnChange := TreeView.OnChange;
  LastFocused := TreeView.Selected;
  TreeView.OnChanging := nil;
  ...
  try
    if (Destination = nil) or not Destination.HasAsParent(Self) then
      begin
        D:=Destination;
        D.Assign(Self);
        ChangesList.Add(D, opInsert);
        S:=Self.GetNext;
        StartLevel:=Self.Level;
        CurLevel:=StartLevel;
        while (S<>nil) and (StartLevel<S.Level) do
        begin
          if S.Level>CurLevel
            then D:=D.Owner.AddChild(D, S.Text)
            else
          if S.Level=CurLevel
            then D:=D.Owner.Add(D, S.Text)
            else begin
              for i:=1 to CurLevel-S.Level do
                D:=D.Parent;
              D:=D.Owner.Add(D, S.Text);
            end;
          D.Assign(S);
          ChangesList.Add(D, opAdd);
          CurLevel:=S.Level;
          S:=S.GetNext;
        end;
      end;
...
begin
  Result := 0;
  Node := FParent;
  while (Node<>nil) and (Node <> FOwner.FRoot) do
  begin
    Inc(Result);
    Node := Node.FParent;
  end;
  ...
  if assigned(TTreeNTNode(Item1).TreeView.FOnCompare)
    then TTreeNTNode(Item1).TreeView.FOnCompare(TTreeNTNode(Item1).TreeView, TTreeNTNode(Item1), TTreeNTNode(Item2), Data, Result)
      ...
      case TTreeNTNode(Item1).TreeView.SortType of
        stData:
          Result := Integer(TTreeNTNode(Item1).FaData) - Integer(TTreeNTNode(Item2).FaData);
        stBoth:
          begin
            Result := CompareText(TTreeNTNode(Item1).FText, TTreeNTNode(Item2).FText);
            if Result = 0 then Result := Integer(TTreeNTNode(Item1).FaData) - Integer(TTreeNTNode(Item2).FaData);
          end;
      else
...
begin
  ...
  begin
    ...
    FCheckType := Node.CheckType;
    FCheckState := Node.CheckState;
    ...
    bDATA:=Node.bDATA;                           // *
    sDATA:=Node.sDATA;                           // *
    iCatName:=Node.iCatName;                     // *
    bCatName:=Node.bCatName;                     // *
    iCatID:=Node.iCatID;                         // *
    bCatID:=Node.bCatID;                         // *
...
procedure TTreeNTNode.SetSingleCheckType(ToType: TCheckType);
var i: smallint;
begin
  if FCheckType <> ToType then
  begin
    if assigned(FOwner) then TreeView.Changed:=true;
    FCheckType := ToType;
    FCheckState := csUnchecked;
    ...
    case ToType of
      ctNone:
        TreeView.SetCheckImage(Self, ckEmpty);
      ctCheckBox, ctCheckBoxGrayed:
        TreeView.SetCheckImage(Self, ckCheckEmpty);
      ctRadioButton:
        if FParent<>nil
          then begin
//            FCheckType:=ctRadioButton;
            for I := 0 to FParent.Count - 1 do
              if (FParent.Item[I].CheckType=ctRadioButton)
              then if FCheckState = csChecked
                     then TreeView.SetCheckImage(Self, ckRadioChecked)
                     else if FParent.Item[I].Enabled
                            then TreeView.SetCheckImage(FParent.Item[I], ckRadioEmpty)
                            else TreeView.SetCheckImage(FParent.Item[I], ckRadioDisabled);
          end;
    end;
...
procedure TTreeNTNode.SetStructureLocked(ToState: boolean);      // *
begin
  FStructureLocked:=ToState;
{  if TreeView.Items.Count>0
    then ChangesList[GetItemID]:=ChangeRec(GetItemID, opChange); }
end;

procedure TTreeNTNode.SetStructureLock(ToState: boolean);        // *
var T: TTreeNTNode;
    StartLevel: smallint;
begin
  FStructureLocked:=ToState;
  ...
  T:=Self.GetNext;
  StartLevel:=Self.Level;
  while (T<>nil) and (T.Level > StartLevel) do
    begin
      T.FStructureLocked:=ToState;
      ChangesList.Add(T, opChange);
      T:=T.GetNext;
    end;
  if assigned(FOwner) then TreeView.Changed:=true;
end;
...
begin
  ...
  FCheckType := Info.CheckType;
  FCheckState := Info.CheckState;
  Enabled := Info.Enabled;
  if Info.Selected
    then FOwner.SelectNode(Self, stSet);
  ...
  bDATA:=Info.bDATA;                           // *
  sDATA:=Info.sDATA;                           // *
  bCatName:=Info.bCatName;                     // *
  iCatName:=Info.iCatName;                     // *
  StructureLocked:=Info.StructureLocked;       // *
  Data := Info.Data;
...
begin
  ...
  Info.CheckType := FCheckType;
  Info.CheckState := FCheckState;
  ...
  Info.bDATA:=bDATA;                         // *
  Info.sDATA:=sDATA;                         // *
  Info.bCatName:=bCatName;                   // *
  Info.iCatName:=iCatName;                   // *
  Info.StructureLocked:=StructureLocked;     // *
...
//      R.Right:=R.Right-Owner.BorderWidth;
...
begin
  ...
  try
    ...
    for I := 0 to Count - 1 do
      with Add(nil, '') do begin
        ReadData(Stream, @NodeInfo);
//        SetSingleCheckType(NodeInfo.CheckType);
      end;
  finally
    EndUpdate;
  end;
...
begin
  ...
  FStdMenu.Name:='DefTreeMenu';
  M := TMenuItem.Create(Self);
  M.Caption := 'New item';
  M.Tag:=1;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(VK_DOWN, [ssCtrl]);
  FStdMenu.Items.Add(M);
  M := TMenuItem.Create(Self);
  M.Caption := 'New child item';
  M.Tag:=2;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(VK_RIGHT, [ssCtrl]);
  FStdMenu.Items.Add(M);
  M := TMenuItem.Create(Self);
  M.Caption := 'Copy tree';
  M.Tag:=3;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(VK_INSERT, [ssCtrl]);
  FStdMenu.Items.Add(M);
  M := TMenuItem.Create(Self);
  M.Caption := 'Paste tree';
  M.Tag:=4;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(VK_INSERT, [ssShift]);
  FStdMenu.Items.Add(M);
  M := TMenuItem.Create(Self);
  M.ShortCut:=ShortCut(VK_INSERT, [ssCtrl, ssShift]);
  M.Caption := 'Paste as child tree';
  M.Tag:=5;
  M.OnClick:=MenuItemClick;
  FStdMenu.Items.Add(M);
  M := TMenuItem.Create(Self);
  M.Caption := 'Delete tree';
  M.Tag:=6;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(VK_DELETE, [ssCtrl]);
  FStdMenu.Items.Add(M);
  ...
  M := TMenuItem.Create(Self);
  M.Caption := 'Set item to RADIO BUTTON';
  M.Tag:=17;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('R'), [ssCtrl]);
  FStdMenu.Items.Add(M);

  M := TMenuItem.Create(Self);
  M.Caption := 'Set item to CHECK BOX';
  M.Tag:=18;
  M.OnClick:=MenuItemClick;
  ...
  M := TMenuItem.Create(Self);
  M.Caption := 'Set item to NONE';
  M.Tag:=19;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('N'), [ssCtrl]);
  FStdMenu.Items.Add(M);
  ...
  M := TMenuItem.Create(Self);
  M.Caption := 'Set childs to RADIO BUTTONS';
  M.Tag:=7;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('R'), [ssCtrl,ssShift]);
  FStdMenu.Items.Add(M);

  M := TMenuItem.Create(Self);
  M.Caption := 'Set childs to CHECK BOXES';
  M.Tag:=8;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('C'), [ssCtrl,ssShift]);
  FStdMenu.Items.Add(M);

  M := TMenuItem.Create(Self);
  M.Caption := 'Set childs to NONE';
  M.Tag:=9;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('N'), [ssCtrl,ssShift]);
  FStdMenu.Items.Add(M);
  ...
  M := TMenuItem.Create(Self);
  M.Caption := 'LOCK Current structure';
  M.Tag:=10;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('L'), [ssCtrl]);
  FStdMenu.Items.Add(M);

  M := TMenuItem.Create(Self);
  M.Caption := 'UNLOCK Current structure';
  M.Tag:=11;
  M.OnClick:=MenuItemClick;
  M.ShortCut:=ShortCut(Word('L'), [ssCtrl,ssShift]);
  FStdMenu.Items.Add(M);

  FStdMenu.OnPopup:=OnStdMenuPopup;

  Self.PopupMenu:=FStdMenu;

  if assigned(AOwner) and (csDesigning in AOwner.ComponentState) then
    FDesigner := GetParentForm(TControl(AOwner)).Designer
                 as {$ifdef DFS_COMPILER_4} IFormDesigner {$else} TFormDesigner {$endif};
  Changed:=false;
  FEditing:=false;

  ChangesList:=TChangesList.Create;
  CanRegisterChanges:=true;
end;



procedure TCustomTreeNT.SetChanged(AState:boolean);
begin
  FTreeChanged:=AState;
end;
...
procedure TCustomTreeNT.OnStdMenuPopup(Sender: TObject);
begin
// 0   New item
// 1   New child item
// 2   Copy tree
// 3   Paste tree
// 4   Paste as child tree
// 5   Delete tree
// 6   -
// 7   Set item to RADIO BUTTON
// 8   Set item to CHECK BOX
// 9   Set item to NONE
// 10  -
// 11  Set childs to RADIO BUTTONS
// 12  Set childs to CHECK BOXES
// 13  Set childs to NONE
// 14  -
// 15  LOCK Current structure
// 16  UNLOCK Current structure
  FStdMenu.Items[0].Enabled:=not Selected.StructureLocked and FMenuEnabled;
  FStdMenu.Items[1].Enabled:=not Selected.StructureLocked and FMenuEnabled;
  FStdMenu.Items[2].Enabled:=FMenuEnabled;
  FStdMenu.Items[3].Enabled:=(FCopiedNode<>nil) and FMenuEnabled;
  FStdMenu.Items[4].Enabled:=(FCopiedNode<>nil) and FMenuEnabled;
  FStdMenu.Items[5].Enabled:=not Selected.StructureLocked and FMenuEnabled;

  FStdMenu.Items[7].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and (Selected.Parent<>nil) and FMenuEnabled;
  FStdMenu.Items[8].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and (Selected.Parent<>nil) and FMenuEnabled;
  FStdMenu.Items[9].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and (Selected.Parent<>nil) and FMenuEnabled;

  FStdMenu.Items[11].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and FMenuEnabled;
  FStdMenu.Items[12].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and FMenuEnabled;
  FStdMenu.Items[13].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and FMenuEnabled;

  FStdMenu.Items[15].Enabled:= FAdvMenuEnabled and not Selected.StructureLocked and FMenuEnabled;
  FStdMenu.Items[16].Enabled:=FAdvMenuEnabled and Selected.StructureLocked and FMenuEnabled;
end;

procedure TCustomTreeNT.MenuItemClick(Sender: TObject);
begin
  if (Selected=nil) or Editing or not FMenuEnabled then Exit;
  try
    with Sender as TMenuItem do
      case Tag of
        1,2 : begin
                case Tag of
                  1 : Selected:=Selected.Owner.Add(Selected, 'New one');
                  2 : Selected:=Selected.Owner.AddChild(Selected, 'New one');
                end;
                Changed:=true;
                Selected.Focused:=true;
                SetFocus;
                Selected.EditText;
                ChangesList.Add(Selected, opAdd);
              end;
                // ShowMessage('Copy tree');
        3 : Selected.CopyNode;
                // ShowMessage('Paste tree');
        4 : if not Selected.StructureLocked
              then begin
                { changes inside  -  ChangesList.Add(Self, opChange); }
                FCopiedNode.PasteNodesTo(Selected.Owner.Insert(Selected, FCopiedNode.Text));
                Changed:=true;
              end;
                // ShowMessage('Paste as child tree');
        5 : if not Selected.StructureLocked
              then begin
                { changes inside  -  ChangesList.Add(Self, opChange); }
                FCopiedNode.PasteNodesTo(Selected.Owner.AddChild(Selected, FCopiedNode.Text));
                Changed:=true;
              end;
        6 : if not Selected.StructureLocked
              then begin
                ChangesList.Add(Selected, opDelete);
                Selected.Delete;
                Changed:=true;
              end;
        7..9 : if Selected.HasChildren and not Selected.StructureLocked then
                 begin
                   { changes inside  -  ChangesList.Add(Self, opChange); }
                   case Tag of
                     7 : Selected.SetCheckType(ctRadioButton);
                     8 : Selected.SetCheckType(ctCheckBox);
                     9 : Selected.SetCheckType(ctNone);
                   end;
                   Changed:=true;
                 end;
        { changes inside  -  ChangesList.Add(Self, opChange); }
        10 : if FAdvMenuEnabled then Selected.SetStructureLock(true);
        11 : if FAdvMenuEnabled then Selected.SetStructureLock(false);
        17..19 : begin
                   case Tag of
                     17 : Selected.SetSingleCheckType(ctRadioButton);
                     18 : Selected.SetSingleCheckType(ctCheckBox);
                     19 : Selected.SetSingleCheckType(ctNone);
                   end;
                   Repaint;
                   Changed:=true;
                 end;
...
procedure TCustomTreeNT.SetMenuEnabled(abState: boolean);
begin
  if abState
    then Self.PopupMenu:=FStdMenu
    else Self.PopupMenu:=nil;
  FMenuEnabled:=Self.PopupMenu<>nil;
end;
    ...
    TVN_BEGINLABELEDIT:
      ...
      begin
        ...
        begin
          ...
          FEditing:=true;
          FStdMenu.Items[0].ShortCut:=0;
          FStdMenu.Items[1].ShortCut:=0;
          FStdMenu.Items[2].ShortCut:=0;
          FStdMenu.Items[3].ShortCut:=0;
          FStdMenu.Items[5].ShortCut:=0;
          FStdMenu.Items[8].ShortCut:=0;
        end;
      end;
    TVN_ENDLABELEDIT:
      begin
        ...
        FEditing:=false;
        FStdMenu.Items[0].ShortCut:=ShortCut(VK_DOWN, [ssCtrl]);
        FStdMenu.Items[1].ShortCut:=ShortCut(VK_RIGHT, [ssCtrl]);
        FStdMenu.Items[2].ShortCut:=ShortCut(VK_INSERT, [ssCtrl]);
        FStdMenu.Items[3].ShortCut:=ShortCut(VK_INSERT, [ssShift]);
        FStdMenu.Items[5].ShortCut:=ShortCut(VK_DELETE, [ssCtrl]);
        FStdMenu.Items[8].ShortCut:=ShortCut(Word('C'), [ssCtrl]);
      end;
    TVN_ITEMEXPANDING:
...
begin
  ...
  if (htOnItem in HitInfo) or (htOnButton in HitInfo) then Message.Result := 1
                                                      else
  begin
    Message.Result := 0;
    if IsEditing then Selected.EndEdit(True);
  end;
end;
...
function TCustomTreeNT.GetDragImages: {$ifdef DFS_COMPILER_4} TDragImageList {$else} TCustomImageList {$endif};
...
// design time and dragging need special handling
// we must overcome the immediate drag start (happens by default in
// TControl.WndProc, this is the special case) and we need the drag messages
// also at design time
...
begin
  // one specific case is important
  SpecialCase := ((Message.Msg = WM_LBUTTONDOWN) or
                  (Message.Msg = WM_LBUTTONDBLCLK)) and
                   not Dragging and (DragMode = dmAutomatic);

  // determine if we can access the designer
  IsDesigning := (csDesigning in ComponentState) and assigned(FDesigner);

  // do we actually have the special case?
  if SpecialCase then
  begin
    // are we at run time or did the user hit an area (at design time)
    // which shouldn't be handled by the default WndProc?
    if not IsDesigning or (IsDesigning and not FDesigner.IsDesignMsg(Self, Message)) then
  ...
  begin
    // this is the branch mostly taken
    // avoid that the ancestors swallow the drag messages
    // (we just do what the ancestors would do at run time, but also at design time)
...
begin
  ...
  begin
    GetCursorPos(P);
    with ScreenToClient(P) do aDragNode := GetNodeAt(X, Y);
  end;

  if aDragNode <> nil then
  begin
    DragNode:=aDragNode;
    ImageHandle := TreeView_CreateDragImage(Handle, aDragNode.ItemID);
...
begin
  ...
  if assigned(FOnEditing)
    then FOnEditing(Self, Node, Result);
end;
...
procedure TCustomTreeNT.LoadFromDB(const TableName: String; const WhereClause: string='');
var aCharData: PChar;
    CharData: string;
    LastNode,
    CurNode: TTreeNTNode;
  ...
  function TC(aCount:byte): string;
  begin
    Result:='';
    while length(Result)<aCount do Result:=Result+#9;
  end;
...
(*  if FConnQuery=nil
    then exit;
  ChangesList.Clear;
  CanRegisterChanges:=false;

  Screen.Cursor:=crSQLWait;
  with FConnQuery do
  begin
    try
      SQL.Text:='select NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName from '+TableName+' '+
                WhereClause+
                ' order by NodeID';
      if assigned(rSQL)
        then rSQL.Assign(SQL);
      Open;
      if FieldByName('NODEID').AsString='0'
        then TNTRootNODE:=''
        else TNTRootNODE:=FieldByName('NODEID').AsString;
    except
        raise Exception.Create('Cannot open query: '+SQL.Text);

{      try
        SQL.Clear;
        SQL.Add('create table '+TableName+' (');
        SQL.Add('    NodeID varchar(25) binary NOT NULL,');
        SQL.Add('    Caption varchar(64),');
        SQL.Add('    CheckType smallint,');
        SQL.Add('    CheckState smallint,');
        SQL.Add('    iDATA integer,');
        SQL.Add('    fDATA numeric(15,5),');
        SQL.Add('    bDATA smallint,');
        SQL.Add('    sDATA varchar(128),');
        SQL.Add('    AddDATA smallint,');
        SQL.Add('    iCatID integer,');
        SQL.Add('    iCatName varchar(30),');
        SQL.Add('    bCatID smallint,');
        SQL.Add('    bCatName varchar(30),');
        SQL.Add('    primary key (NodeID))');
        ExecSQL;
      except
        Screen.Cursor:=crDefault;
        raise Exception.Create('Connection error or invalid table name! Exiting.');
      end;
...
//    if TNTRootNODE<>''
//      then Next;
    while not FConnQuery.EOF do begin
      if TNTRootNODE=''
        then CharData:=CharData+TC(length(FConnQuery.FieldByName('NodeID').AsString)-1)+FConnQuery.FieldByName('Caption').AsString+#13
        else if FConnQuery.FieldByName('NodeID').AsString=TNTRootNODE
               then CharData:=FConnQuery.FieldByName('Caption').AsString+#13
               else CharData:=CharData+TC(length(FConnQuery.FieldByName('NodeID').AsString)-length(TNTRootNODE))+FConnQuery.FieldByName('Caption').AsString+#13;
      Next;
    end;
  end;
  ...
  try
    ...
    begin
      Clear;
      CurrentLine := 1;
      GetMem(aCharData, length(CharData)+1);
      StrPCopy(aCharData, CharData);
      FRoot.ReadStrings(aCharData, 0);
...
{      if TNTRootNODE<>''
      ...
      while (CurNode<>nil) and (not FConnQuery.EOF) do
        begin
          if CurNode.GetItemID=FConnQuery.FieldByName('NodeID').AsString
            then begin
              CurNode.SetText(FConnQuery.Fields[1].AsString);
              CurNode.FCheckType:=TCheckType(FConnQuery.Fields[2].AsInteger);
  // SetSingleCheckType();
              CurNode.CheckState:=TCheckState(FConnQuery.Fields[3].AsInteger);
              CurNode.iDATA:=FConnQuery.Fields[4].AsInteger;
              CurNode.fDATA:=FConnQuery.Fields[5].AsFloat;
              CurNode.bDATA:=FConnQuery.Fields[6].AsInteger;
              CurNode.sDATA:=FConnQuery.Fields[7].AsString;
              if not FConnQuery.Fields[8].IsNull
                then CurNode.SetStructureLock((FConnQuery.Fields[8].AsInteger and 1) > 0);
              CurNode.iCatID:=FConnQuery.Fields[9].AsInteger;
              CurNode.iCatName:=FConnQuery.Fields[10].AsString;
              CurNode.bCatID:=FConnQuery.Fields[11].AsInteger;
              CurNode.bCatName:=FConnQuery.Fields[12].AsString;
            end;
          FConnQuery.Next;
          CurNode:=CurNode.GetNext;
        end;
...
*)
  ...
  begin
    ...
    Items.Clear;
    LastNode := nil;
    RootLevel := length(TNTRootNODE);

    while not FConnQuery.EOF do
    begin
      if LastNode=nil
        then CurNode := Items.AddChild(LastNode, FConnQuery.Fields[1].AsString)
        else if LastNode.Level < length(FConnQuery.Fields[0].AsString)-RootLevel-1  // Если надо "ребёнка"
               then CurNode := Items.AddChild(LastNode, FConnQuery.Fields[1].AsString)
               else
             if LastNode.Level = length(FConnQuery.Fields[0].AsString)-RootLevel-1  // Если надо элемент на том-же уровне
               then CurNode := Items.Add(LastNode, FConnQuery.Fields[1].AsString)
               else
             if LastNode.Level > length(FConnQuery.Fields[0].AsString)-RootLevel-1
               then begin
                 while LastNode.Level > length(FConnQuery.Fields[0].AsString)-RootLevel-1 do
                   LastNode:=LastNode.Parent;
                 CurNode := Items.Add(LastNode, FConnQuery.Fields[1].AsString);
               end;
...
procedure TCustomTreeNT.SaveToDB(const TableName: String; const WhereClause: string=''; const LimitNode: string='');
var CurNode: TTreeNTNode;
    ExecErrors: smallint;
  function TC(aCount:byte): string;
  begin
    Result:='';
...
begin
  ...
  with FConnQuery do
  begin
//    if not FConnQuery.Active
//      then FConnQuery.Active:=true;
  // checking database
  if WhereClause=''
    then begin
      SQL.Text:='drop table if exists '+TableName+'_tmp';
      ExecSQL;
      SQL.Clear;
      SQL.Add('create table '+TableName+'_tmp (');
      SQL.Add('    NodeID varchar(25) binary NOT NULL,');
      SQL.Add('    Caption varchar(40),');
      SQL.Add('    CheckType smallint,');
      SQL.Add('    CheckState smallint,');
      SQL.Add('    iDATA integer,');
      SQL.Add('    fDATA numeric(15,5),');
      ...
      SQL.Text:='delete from '+TableName+' '+WhereClause;
      ExecSQL;
    end;
{
    S:=TStringList.Create;
    for i:=0 to high(ChangesList.List) do
      S.Add(ChangesList.GenSQL(i, TableName));
    S.SaveToFile('d:\'+TableName+'.txt');
    S.Free;
    ChangesList.Free;
}
    Screen.Cursor:=crSQLWait;
        ...
        if WhereClause=''
          then begin
            ...
            while CurNode<>nil do
              begin
                SQL.Clear;
                SQL.Add('insert into '+TableName+'_tmp (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)');
                SQL.Add(Format('values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s")',
                    [CurNode.GetItemID, SQLQuotes(CurNode.Text), byte(CurNode.CheckType),
                     byte(CurNode.CheckState), CurNode.iDATA, CurNode.fDATA,
                     CurNode.bDATA, SQLQuotes(CurNode.sDATA), byte(CurNode.StructureLocked) or byte(CurNode.Index) shl 1,
                     CurNode.iCatID, CurNode.iCatName, CurNode.bCatID, CurNode.bCatName]));
                try
                  ExecSQL;
                except
                  ShowMessage('Error executing query:'#13+SQL.Text);
                  inc(ExecErrors);
                end;
                CurNode:=CurNode.GetNext;
              end;
            ...
            while CurNode<>nil do
              begin
                if ((pos(LimitNode, CurNode.GetItemID)=1) and (LimitNode<>'')) or
                   (LimitNode='')
                  then begin
                    SQL.Clear;
                    SQL.Add('insert into '+TableName+' (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)');
                    SQL.Add(Format('values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s")',
                        [CurNode.GetItemID, SQLQuotes(CurNode.Text), byte(CurNode.CheckType),
                         byte(CurNode.CheckState), CurNode.iDATA, CurNode.fDATA,
                         CurNode.bDATA, SQLQuotes(CurNode.sDATA), byte(CurNode.StructureLocked) or byte(CurNode.Index) shl 1,
    ...
    if ExecErrors>0
      then if MessageDlg(Format('Encountered %d errors while saving data!'#13+
                         'Abort saving?', [ExecErrors]), mtWarning, [mbYes,mbNo], 0)=mrYes
             then exit;
    Screen.Cursor:=crSQLWait;
    if WhereClause=''
      then begin
        ...
        ExecSQL;
        SQL.Text:='alter table '+TableName+'_tmp rename as '+TableName;
        ExecSQL;
      end;
    SQL.Text:='flush tables';
    ExecSQL;
    Screen.Cursor:=crDefault;
    Changed:=false;
  end;
...
procedure TCustomTreeNT.SaveToFile(const TableName: String; const WhereClause: string=''; const LimitNode: string='');
var CurNode: TTreeNTNode;
    FFile: TStringList;
  function TC(aCount:byte): string;
  begin
...
//      then FConnQuery.Active:=true;
  ...
  if WhereClause=''
    then begin
      FFile.Text:='drop table if exists Setup;';
      FFile.Add('create table Setup (');
      FFile.Add('    NodeID varchar(25) binary NOT NULL,');
      FFile.Add('    Caption varchar(40),');
      FFile.Add('    CheckType smallint,');
      FFile.Add('    CheckState smallint,');
      FFile.Add('    iDATA integer,');
      FFile.Add('    fDATA numeric(15,5),');
      FFile.Add('    bDATA smallint,');
      FFile.Add('    sDATA varchar(128),');
      FFile.Add('    AddDATA smallint,');
      FFile.Add('    iCatID integer,');
      FFile.Add('    iCatName varchar(30),');
      FFile.Add('    bCatID smallint,');
      FFile.Add('    bCatName varchar(30),');
      FFile.Add('    primary key (NodeID));');
    end
    else begin
      FFile.Text:='delete from Setup '+WhereClause;
    end;
{
    S:=TStringList.Create;
    for i:=0 to high(ChangesList.List) do
  ...
  begin
    if WhereClause=''
      then begin
        ...
        while CurNode<>nil do
          begin
            FFile.Add('insert into Setup (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)'+
                    Format('values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s");',
                      [CurNode.GetItemID, SQLQuotes(CurNode.Text), byte(CurNode.CheckType),
                       byte(CurNode.CheckState), CurNode.iDATA, CurNode.fDATA,
                       CurNode.bDATA, SQLQuotes(CurNode.sDATA), byte(CurNode.StructureLocked) or byte(CurNode.Index) shl 1,
                       CurNode.iCatID, CurNode.iCatName, CurNode.bCatID, CurNode.bCatName]));
        ...
        while CurNode<>nil do
          begin
            if ((pos(LimitNode, CurNode.GetItemID)=1) and (LimitNode<>'')) or
               (LimitNode='')
              then FFile.Add('insert into Setup (NodeID, Caption, CheckType, CheckState, iDATA, fDATA, bDATA, sDATA, AddDATA, iCatID, iCatName, bCatID, bCatName)'+
                    Format('values ("%s", "%s", %d, %d, %d, %5.4f, %d, "%s", %d, %d, "%s", %d, "%s");',
                      [CurNode.GetItemID, SQLQuotes(CurNode.Text), byte(CurNode.CheckType),
                       byte(CurNode.CheckState), CurNode.iDATA, CurNode.fDATA,
                       CurNode.bDATA, SQLQuotes(CurNode.sDATA), byte(CurNode.StructureLocked) or byte(CurNode.Index) shl 1,
  ...
  Screen.Cursor:=crDefault;
  Changed:=false;
  FFile.SaveToFile(TableName+'.sql');
  FFile.Free;
end;
...
begin
  ...
  begin
    ...
    if FDesignerMode and
       ...
       (TObject(Node.Data) is TPersistent) then
    begin
      Allowed := True;
      if assigned(FOnDesignClick) then FOnDesignClick(Self, Node, Allowed);
      {$ifdef DFS_COMPILER_3_UP}
        if allowed then FDesigner.SelectComponent(TPersistent(Node.Data));
      {$else}
...
function TCustomTreeNT.GetItemID: string;
begin
  if GetSelection=nil
    then Result:=''
    else Result:=GetSelection.GetItemID;
end;

procedure TCustomTreeNT.SetQueryValue(a:TDataSet);
begin
  if a<>nil
    then FConnQuery:=a
    else FConnQuery:=nil;
...
procedure TCustomTreeNT.WMPaint(var Message: TWMPaint);

begin
  GetUpdateRect(Handle, FUpdateRect, false);
//  FUpdateRect.Right:=FUpdateRect.Right+30;//BorderWidth;
//  GetUpdateRect(Handle, FUpdateRect, false);
  inherited;
end;
